...
class UserAuth
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if($request->path()=="9851240938/seller" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        ...
        if($request->path()=="9851240938/banner/add-homepage-banner" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/banner/add-user-dashboard-banner" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/products" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/unverified-products" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/orders/buyer" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/orders/seller" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/product/add-product" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/category/details" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/{name}/subcategory/details" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
        if($request->path()=="9851240938/seller/edit/{id}" && !$request->session()->has('admin')){
            return redirect()->route('admin-login');
        }
